...
obs_probe="$root/monitoring/alloy/observability-probe.alloy"
...
grep -Fq '/etc/alloy/observability-probe.alloy' "$tasks"
grep -Fq "alloy_node_role == 'observability'" "$tasks"
grep -Fq "alloy_node_role != 'observability'" "$tasks"
...
grep -Fq 'prometheus.exporter.blackbox "application"' "$obs_probe"
grep -Fq 'name    = "application-api"' "$obs_probe"
grep -Fq 'address = "http://10.40.0.20:8080/api/v1/programs?size=1"' "$obs_probe"
grep -Fq 'discovery.relabel "application_probe"' "$obs_probe"
grep -Fq 'prometheus.scrape "application_probe"' "$obs_probe"
grep -Fq 'forward_to      = [prometheus.remote_write.central.receiver]' "$obs_probe"
