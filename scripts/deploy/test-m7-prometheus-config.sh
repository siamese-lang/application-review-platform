...
grep -Fq 'prometheus_memory_limit: 1536M' "$vars"
grep -Fq 'prometheus_cpu_quota: 75%' "$vars"
...
grep -Fq 'MemoryMax={{ prometheus_memory_limit }}' "$unit"
grep -Fq 'CPUQuota={{ prometheus_cpu_quota }}' "$unit"
