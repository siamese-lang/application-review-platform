...
if [[ -z ${ARP_OSLOGIN_USER:-} || -z ${ARP_OSLOGIN_SSH_KEY:-} || -z ${ARP_OSLOGIN_KNOWN_HOSTS:-} ]]; then
  : "${ARP_CONFIRM_M10_DATASET_RESET:?Set ARP_CONFIRM_M10_DATASET_RESET=yes for the guarded R3a reset}"
...
if not match:
    raise SystemExit("M10 R3a could not capture dataset M manifest SHA-256")
print(match.group(1))
PY
  )
  ...
  exec "$root/deploy/with-oslogin-ssh.py" \
    --ttl-seconds 1800 \
    -- bash "$root/scripts/reliability/run-m10-r3a.sh"
fi

[[ ${ARP_M10_DATASET_READY:-} == yes ]] || {
  echo "ERROR: M10 R3a must enter trusted SSH phase after dataset verification." >&2
  exit 1
}
[[ ${ARP_M10_DATASET_MANIFEST_SHA:-} =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: M10 R3a verified dataset manifest SHA-256 is missing." >&2
  exit 1
}
...
echo "STEP: verify post-R2 database and application health"
...
if values.get("Id") != "arp.service" or values.get("ActiveState") != "active" or values.get("SubState") != "running":
    raise SystemExit(f"application not healthy before R3a: {values!r}")
...
run_id="m10-r3a-$(date -u +%Y%m%dT%H%M%SZ)-${actual_sha:0:8}"
...
node2_full=$("${ssh_s2[@]}" sudo docker exec garage /garage -c /etc/garage.toml node id -q)
node2_id=${node2_full%%@*}
[[ $node2_id =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: invalid storage-02 Garage node ID: $node2_full" >&2
  exit 1
}
node2_short=${node2_id:0:16}

"${ssh_s1[@]}" sudo docker exec garage /garage -c /etc/garage.toml status \
  > "$run_dir/garage-status-before.txt" 2>&1

python3 - "$run_dir/garage-status-before.txt" "$node2_short" <<'PY'
import sys
...
if short not in healthy:
    raise SystemExit("storage-02 is not healthy before R3a")
print("M10_R3A_PREFAULT_STORAGE02_HEALTHY=PASS")
PY

remote_dir=$("${ssh_loadgen[@]}" mktemp -d /tmp/arp-m10-r3a.XXXXXX)
[[ $remote_dir == /tmp/arp-m10-r3a.* ]] || {
...
cleanup() {
  ...
  if (( garage_fault_active == 1 )); then
    echo "EMERGENCY_RECOVERY: storage-02 Garage may still be stopped; starting it." >&2
    "${ssh_s2[@]}" sudo docker start garage >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT

tar -C "$root/workload" -cf - k6/m10-r3a-garage-non-endpoint.js fixtures/w1-attachment.txt |
...
remote_command="set -euo pipefail; IFS= read -r APPLICANT_PASSWORD; export APPLICANT_PASSWORD; cd '$remote_dir'; BASE_URL='$base_url' RUN_ID='$run_id' M10_DURATION='4m' APPLICANT_USERNAME='m6-applicant' ATTACHMENT_PATH='../fixtures/w1-attachment.txt' k6 run --summary-export summary.json k6/m10-r3a-garage-non-endpoint.js"

echo "STEP: start R3a attachment and non-attachment probes"
(
...
echo "STEP: wait for healthy pre-fault R3a probes"
ready=0
for _ in $(seq 1 180); do
  if grep -q 'M10_R3A_ATTACHMENT_OK' "$k6_output" 2>/dev/null &&
     grep -q 'M10_R3A_NON_ATTACHMENT_OK' "$k6_output" 2>/dev/null; then
    ready=1
    break
  fi
  sleep 0.25
...
[[ $ready == 1 ]] || {
  echo "ERROR: R3a probes did not establish a healthy pre-fault baseline." >&2
...
echo "STEP: stop storage-02 Garage container at $fault_at"
garage_fault_active=1
"${ssh_s2[@]}" sudo docker stop --time 10 garage >/dev/null

stopped_at=$(
  "${ssh_s2[@]}" bash -s <<'REMOTE'
set -euo pipefail
...
echo "storage-02 Garage did not stop within 20 seconds" >&2
exit 1
REMOTE
)
echo "M10_R3A_STORAGE02_STOPPED_AT=$stopped_at"

echo "STEP: hold bounded non-endpoint node outage for 60 seconds"
sleep 20
...
for key in sorted(values):
    print(f"M10_R3A_DURING_{key.upper()}={values[key]}")
PY

sleep 40

restore_at=$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)
echo "STEP: start storage-02 Garage container at $restore_at"
...
echo "storage-02 did not return to Garage healthy nodes within 60 seconds" >&2
exit 1
REMOTE
)
garage_fault_active=0
echo "M10_R3A_STORAGE02_HEALTHY_AT=$recovered_at"
...
k6_job=""
if [[ $k6_rc -ne 0 ]]; then
  echo "ERROR: R3a k6 probe process exited with rc=$k6_rc" >&2
...
attachment_attempts = value("m10_r3a_attachment_attempts", "count")
attachment_errors = value("m10_r3a_attachment_errors", "value")
upload_errors = value("m10_r3a_attachment_upload_errors", "value")
download_errors = value("m10_r3a_attachment_download_errors", "value")
delete_errors = value("m10_r3a_attachment_delete_errors", "value")
non_attachment_attempts = value("m10_r3a_non_attachment_attempts", "count")
non_attachment_errors = value("m10_r3a_non_attachment_errors", "value")
support_errors = value("m10_r3a_support_errors", "value")

if attachment_attempts <= 0 or non_attachment_attempts <= 0:
    raise SystemExit("R3a probe recorded no attempts")

supported = (
    attachment_errors == 0.0
    and non_attachment_errors == 0.0
    and support_errors == 0.0
)

print(f"M10_R3A_ATTACHMENT_ATTEMPTS={int(attachment_attempts)}")
print(f"M10_R3A_ATTACHMENT_ERROR_RATE={attachment_errors:.6f}")
print(f"M10_R3A_ATTACHMENT_UPLOAD_ERROR_RATE={upload_errors:.6f}")
print(f"M10_R3A_ATTACHMENT_DOWNLOAD_ERROR_RATE={download_errors:.6f}")
print(f"M10_R3A_ATTACHMENT_DELETE_ERROR_RATE={delete_errors:.6f}")
print(f"M10_R3A_NON_ATTACHMENT_ATTEMPTS={int(non_attachment_attempts)}")
print(f"M10_R3A_NON_ATTACHMENT_ERROR_RATE={non_attachment_errors:.6f}")
print(f"M10_R3A_SUPPORT_ERROR_RATE={support_errors:.6f}")
print(f"M10_R3A_HYPOTHESIS={'SUPPORTED' if supported else 'NOT_SUPPORTED'}")
PY
)
...
if not pg or min(pg) != 1.0:
    raise SystemExit(f"PostgreSQL availability changed during R3a: {pg!r}")
if not app or min(app) != 1.0:
    raise SystemExit(f"application blackbox probe changed during R3a: {app!r}")
...
if min(garage["storage-01"]) != 1.0 or min(garage["storage-03"]) != 1.0:
    raise SystemExit(f"unrelated Garage endpoint/peer node went down: {garage!r}")
if min(garage["storage-02"]) != 0.0 or max(garage["storage-02"]) != 1.0:
    raise SystemExit(f"storage-02 telemetry did not capture outage and recovery: {garage['storage-02']!r}")

print("M10_R3A_POSTGRES_UP_MIN=1")
print("M10_R3A_APPLICATION_PROBE_MIN=1")
for node in ["storage-01", "storage-02", "storage-03"]:
    print(f"M10_R3A_GARAGE_{node.upper().replace('-', '_')}_MIN={min(garage[node]):.0f}")
    print(f"M10_R3A_GARAGE_{node.upper().replace('-', '_')}_MAX={max(garage[node]):.0f}")
print("M10_R3A_TELEMETRY_NODE_ISOLATION=PASS")
PY
)
...
if values.get("ActiveState") != "active" or values.get("SubState") != "running":
    raise SystemExit(f"application not healthy after R3a: {values!r}")
if int(values["MainPID"]) != before_pid:
    raise SystemExit(f"application PID changed during R3a: {before_pid} -> {values['MainPID']}")
if int(values.get("NRestarts", "0")) != before_restarts:
    raise SystemExit("application restart count changed during R3a")
print(f"M10_R3A_APP_PID_BEFORE={before_pid}")
print(f"M10_R3A_APP_PID_AFTER={values['MainPID']}")
print("M10_R3A_APPLICATION_STABLE=PASS")
PY

echo "STEP: post-R3a HTTPS business smoke"
BASE_URL="$base_url" \
...
echo "STEP: post-R3a database business invariants"
cat "$root/scripts/reliability/m10-db-invariants.sql" |
  "${ssh_db[@]}" sudo -u postgres psql -d arp -v ON_ERROR_STOP=1 \
...
for key in sorted(values):
    print(f"M10_R3A_POST_{key.upper()}={values[key]}")

clean = all(values.get(key, 0) == 0 for key in lifecycle)
print("M10_R3A_DB_CORE_INVARIANTS=PASS")
print(f"M10_R3A_ATTACHMENT_LIFECYCLE_CLEAN={'PASS' if clean else 'FAIL'}")
PY
)
...
python3 - \
  ...
  "$scenario_finished_at" "$finished_at" "$fixture_sha" "$node2_id" <<'PY'
import json
import sys
...
(
    ...
    started_at, fault_at, stopped_at, restore_at, recovered_at,
    scenario_finished_at, finished_at, fixture_sha, node2_id,
) = sys.argv[1:]
...
manifest = {
    ...
    "workload": {
        "scenario": "m10-r3a-garage-non-endpoint",
        "duration": "4m",
        "attachment_vus": 2,
    ...
    "reliability": {
        ...
        "fault_injected": True,
        "fault_target": "storage-02/garage",
        "fault_node_id": node2_id,
...
echo "M10_R3A_RUN_ID=$run_id"
echo "M10_R3A_SOURCE_SHA=$actual_sha"
echo "M10_R3A_BACKEND_RELEASE_SHA=$backend_release_sha"
echo "M10_R3A_FRONTEND_RELEASE_SHA=$frontend_release_sha"
echo "M10_R3A_DATASET_MANIFEST_SHA256=$ARP_M10_DATASET_MANIFEST_SHA"
echo "M10_R3A_OVERLAY_SHA256=$overlay_sha"
echo "M10_R3A_FAULT_AT=$fault_at"
echo "M10_R3A_RESTORE_AT=$restore_at"
echo "M10_R3A_STORAGE02_HEALTHY_AT=$recovered_at"
echo "M10_R3A_ARTIFACT_DIR=$run_dir"
echo "PASS: M10 R3a experiment retained non-endpoint Garage fault, attachment/non-attachment observations, node telemetry, recovery, business smoke, and DB state."
