...
if [[ -z ${ARP_OSLOGIN_USER:-} || -z ${ARP_OSLOGIN_SSH_KEY:-} || -z ${ARP_OSLOGIN_KNOWN_HOSTS:-} ]]; then
  : "${ARP_CONFIRM_M8_DATASET_RESET:?Set ARP_CONFIRM_M8_DATASET_RESET=yes so W2 can restore dataset M}"
...
if not match:
    raise SystemExit("W2 could not capture the verified dataset M manifest SHA-256")
print(match.group(1))
PY
  )
  ...
  export ARP_M8_W2_DATASET_READY=yes
  export ARP_M8_W2_DATASET_MANIFEST_SHA="$dataset_manifest_sha"

  exec "$root/deploy/with-oslogin-ssh.py"     --ttl-seconds 3600     -- bash "$root/workload/run-w2.sh"
fi

[[ ${ARP_M8_W2_DATASET_READY:-} == yes ]] || {
  echo "ERROR: W2 must enter the trusted SSH phase only after dataset M verification." >&2
  exit 1
}
[[ ${ARP_M8_W2_DATASET_MANIFEST_SHA:-} =~ ^[0-9a-f]{64}$ ]] || {
...
echo "STEP: apply deterministic W2 interactive overlay"
cat "$overlay" |
...
run_id="m8-w2-$(date -u +%Y%m%dT%H%M%SZ)-${actual_sha:0:8}"
...
remote_dir=$("${ssh_loadgen[@]}" mktemp -d /tmp/arp-m8-w2.XXXXXX)
[[ $remote_dir == /tmp/arp-m8-w2.* ]] || {
...
tar -C "$root/workload" -cf -   k6/w2-baseline.js   fixtures/w1-attachment.txt |
...
remote_command="set -euo pipefail; IFS= read -r APPLICANT_PASSWORD; IFS= read -r REVIEWER_PASSWORD; export APPLICANT_PASSWORD REVIEWER_PASSWORD; cd '$remote_dir'; BASE_URL='$base_url' RUN_ID='$run_id' APPLICANT_USERNAME='m6-applicant' REVIEWER_USERNAME='m6-reviewer' ATTACHMENT_PATH='../fixtures/w1-attachment.txt' k6 run --summary-export summary.json k6/w2-baseline.js"

echo "STEP: run W2 mixed normal baseline"
{
...
print(f"W2_BUSINESS_REQUESTS={int(total)}")
for family, target in families:
    observed = counts[family] / total * 100.0
    print(
        f"W2_MIX_{family.upper()}="
        f"count={int(counts[family])} observed_pct={observed:.2f} target_pct={target:.2f}"
    )
print(f"W2_NON_FILE_SUCCESS_RATE={success_rate:.6f}")
print(f"W2_NON_FILE_P95_MS={p95:.3f}")
print(f"W2_REGRESSION_TARGET={'PASS' if target_ok else 'FAIL'}")
PY

echo "W2_RUN_ID=$run_id"
echo "W2_SOURCE_SHA=$actual_sha"
echo "W2_RELEASE_SHA=$release_sha"
echo "W2_BACKEND_RELEASE_SHA=$backend_release_sha"
echo "W2_FRONTEND_RELEASE_SHA=$frontend_release_sha"
echo "W2_DATASET_MANIFEST_SHA256=$ARP_M8_W2_DATASET_MANIFEST_SHA"
echo "W2_OVERLAY_SHA256=$overlay_sha"
echo "W2_ARTIFACT_DIR=$run_dir"
echo "PASS: M8 W2 completed and retained k6, manifest, and pg_stat_statements evidence."
