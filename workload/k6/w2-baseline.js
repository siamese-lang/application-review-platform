...
export function createSave() {
  ...
  const created = businessJson(
    ...
    'POST /api/v1/applications',
    {
      ...
      applicantOrganizationName: 'M8 W2 Synthetic Organization',
      projectTitle: `M8 W2 ${unique}`,
      shortSummary: 'M8 W2 mixed baseline create/save request.',
      requestedAmount: 1234567,
      detailedPlan: 'M8 W2 mixed baseline draft created through the deployed API.',
    },
    [201],
  );
  ...
  if (created.status === 201 && application && application.id !== undefined) {
    businessJson(
      ...
      'PUT /api/v1/applications/{id}',
      {
        ...
        applicantOrganizationName: 'M8 W2 Synthetic Organization',
        projectTitle: `M8 W2 ${unique}`,
        shortSummary: 'M8 W2 mixed baseline saved draft.',
        requestedAmount: 1234567,
        detailedPlan: 'M8 W2 mixed baseline draft saved through the deployed API.',
      },
      [200],
    );
  }
...
export function submitResubmit() {
  ...
  if (slot >= 4000) {
    exec.test.abort(`W2 submit DRAFT pool exhausted at slot ${slot}`);
  }
...
export function reviewAction() {
  ...
  const slot = exec.scenario.iterationInTest;
  if (slot >= 3500) {
    exec.test.abort(`W2 review SUBMITTED pool exhausted at slot ${slot}`);
  }
  ...
  if (startedReview.status === 200 && application && application.version !== undefined) {
    ...
    if (decision === 0) {
      ...
      businessJson(
        ...
        { version: application.version, reason: 'M8 W2 synthetic rejection' },
        [200],
      );
    } else {
      businessJson(
        ...
        { version: application.version, reason: 'M8 W2 synthetic revision request' },
        [200],
      );
    }
  }
...
export function attachment() {
  ...
  const uploaded = http.post(
    ...
    {
      file: http.file(attachmentBody, 'm8-w2-attachment.txt', 'text/plain'),
    },
    {
