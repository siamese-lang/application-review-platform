...
| Immutable release + rollback | OCI artifact, manifest/checksum, versioned release, CI/CD | M4 built from a working tree/local JAR and M5 frontend had no durable paired release identity, so exact redeploy/rollback was not proven | M6 now has live exact-SHA/digest handoff, paired backend/frontend activation, Flyway V5 verification, full HTTPS/API/Garage smoke, a real schema-compatible B→A rollback observed at 38.346 s with no DB down-migration, post-rollback smoke, and final return to Release B; sanitized Phase 5 evidence and an Evidence Card are retained | E3 | Phase 6 must finish manual browser validation, post-deploy no-drift verification, runtime lifecycle disposition, and final closeout/CI before deciding whether this reaches E5 | Primary candidate — high |
