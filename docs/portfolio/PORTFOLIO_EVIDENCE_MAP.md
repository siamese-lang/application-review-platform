...
| PostgreSQL query bottleneck | pg_stat_statements, EXPLAIN ANALYZE BUFFERS, index/query design | Representative peak load may expose query-cost and DB saturation limits | M8 W2/W3 retained dataset-M workload evidence: W2 healthy at 30 VU; W3 offered 100 business req/s dropped about 47.5% of scheduled iterations, non-file p95 2,067.479 ms, db-01 CPU avg about 90.94%, Hikari pending avg about 39.52; reviewer queue result/count and applicant-list SQL costs rose materially | E3 | M9 extracts exact SQL, runs EXPLAIN (ANALYZE, BUFFERS), chooses the smallest evidence-supported change, then repeats equivalent W2/W3 conditions | Measured primary M9 candidate; solution not predetermined |
