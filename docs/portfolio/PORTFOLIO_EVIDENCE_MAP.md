...
| Immutable release + rollback | OCI artifact, manifest/checksum, versioned release, CI/CD | M4 built from a working tree/local JAR and M5 frontend had no durable paired release identity, so exact redeploy/rollback was not proven | M6 Phase 1 publishes one backend/frontend manifest by full main SHA to GHCR with pull-by-digest round-trip verification; Phase 2 adds exact-SHA versioned install, paired backend/frontend preflight, atomic local pointer switching, explicit rollback target/state validation, and an Ansible-controlled final release path with focused CI | E1 | Phase 3 must constrain keyless GitHub→GCP delivery to reviewed exact releases; Phase 5 must prove a real schema-compatible rollback and post-rollback smoke before this becomes a primary story | Primary candidate — high if drill succeeds |
