...
| Immutable release + rollback | OCI artifact, manifest/checksum, versioned release, CI/CD | M4 built from a working tree/local JAR and M5 frontend had no durable paired release identity, so exact redeploy/rollback was not proven | M6 plan identifies gap only | E0 | M6 must retain source SHA/digest/checksums, active release before/after, actual compatible rollback, smoke result, elapsed rollback time | Primary candidate — high if drill succeeds |
