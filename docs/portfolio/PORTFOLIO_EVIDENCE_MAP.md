...
| Immutable release + rollback | OCI artifact, manifest/checksum, versioned release, CI/CD | M4 built from a working tree/local JAR and M5 frontend had no durable paired release identity, so exact redeploy/rollback was not proven | M6 Phase 1 publishes one backend/frontend manifest by full main SHA to GHCR with pull-by-digest round-trip verification; Phase 2 adds exact-SHA versioned install/rollback mechanics and paired preflight; Phase 3 adds immutable-ID constrained GitHub WIF plus an explicit reviewed-main SHA/exact-digest workflow that re-verifies OCI provenance/payloads and can only stage to fixed `ops-01`; Phase 4 applies that WIF/IAM boundary in GCP, recreates the seven-role runtime, establishes controlled state/SOPS/Garage readiness, and finishes with no-drift plans | E1 | Phase 5 must prove an actual GitHub WIF deployment dispatch, immutable release activation, schema-compatible rollback, and post-rollback business smoke before this becomes a primary story | Primary candidate — high if drill succeeds |
