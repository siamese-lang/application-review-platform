...
public class ApplicationService {
 ...
 @Transactional public Application startReview(User actor,long id){requireRole(actor,Role.REVIEWER);Application a=get(id);requireStatus(a,ApplicationStatus.SUBMITTED);if(a.getReviewer()!=null&&!a.getReviewer().getId().equals(actor.getId()))throw new BusinessRuleException("Only the assigned reviewer may resume review");if(a.getReviewer()==null)a.assignReviewer(actor);transition(a,actor,ApplicationStatus.IN_REVIEW,null);return a;}
 ...
 public void requireVisibleToReviewer(User actor,Application a){requireRole(actor,Role.REVIEWER);boolean unassignedSubmission=a.getStatus()==ApplicationStatus.SUBMITTED&&a.getReviewer()==null;boolean assignedToActor=a.getReviewer()!=null&&a.getReviewer().getId().equals(actor.getId());if(!unassignedSubmission&&!assignedToActor)throw new BusinessRuleException("Application is not available to this reviewer");}
