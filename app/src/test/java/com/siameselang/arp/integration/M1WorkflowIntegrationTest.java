...
class M1WorkflowIntegrationTest {
 ...
 @Test void resubmissionStaysWithTheInitiallyAssignedReviewer(){var a=service.create(applicant,program.getId(),"Proposal","Details");service.submit(applicant,a.getId());service.startReview(reviewer,a.getId());service.decide(reviewer,a.getId(),ApplicationStatus.NEEDS_REVISION,"More detail");service.submit(applicant,a.getId());assertThat(service.reviewQueue(reviewer)).contains(a);assertThat(service.reviewQueue(otherReviewer)).doesNotContain(a);assertThatThrownBy(()->service.startReview(otherReviewer,a.getId())).isInstanceOf(BusinessRuleException.class);service.startReview(reviewer,a.getId());assertThat(a.getReviewer()).isEqualTo(reviewer);}
