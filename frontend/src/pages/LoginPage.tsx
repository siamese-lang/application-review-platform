...
export function LoginPage() {
  ...
  return <section className="page auth-page"><div className="auth-intro"><div className="eyebrow">Secure access</div><h1>Welcome back</h1><p>Sign in to your existing account. Your session is protected by the server and is not stored in browser JavaScript.</p></div><form className="auth-card" onSubmit={submit}><h2>Log in</h2>{error !== undefined && <ErrorNotice error={error} />}<label>Username<input name="username" required maxLength={50} autoComplete="username" /></label><label>Password<input name="password" type="password" required maxLength={72} autoComplete="current-password" /></label><button className="button full" disabled={submitting}>{submitting ? 'Signing in…' : 'Log in'}</button><p className="form-footer">Need an applicant account? <Link to="/register">Register</Link></p></form></section>
}
